...
final _projectContract = DeployedContract(
  ContractAbi.fromJson(
...
const _transactionsFileName = 'transactions.csv';
...
const _blockColumn = 1;
const _timestampColumn = 2;
const _fromColumn = 4;
const _valueInColumn = 7;
const _methodColumn = 15;

final client = Web3Client(_nodeUrl, IOClient(HttpClient()));

Future<void> main() async {
  final result = Result();
  final users = Users();

  final txString = File(_transactionsFileName).readAsStringSync();
  ...
  for (final row in txList) {
    final block = int.parse(row[_blockColumn]);
    final secondsSinceEpoch = int.parse(row[_timestampColumn]);
    final from = row[_fromColumn];
    final valueIn = double.parse(row[_valueInColumn]);
    final method = row[_methodColumn];
